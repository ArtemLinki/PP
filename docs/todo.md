...
- [x] Получить API ключ (Groq — бесплатный), добавить в `.env` как `AI_API_KEY`
...
### 4.1.5 Карточка товара (`app/catalog/[slug]/page.tsx`)
- [x] Галерея изображений с миниатюрами
- [x] Хлебные крошки (Главная → Каталог → Товар)
- [x] Название, SKU, badge наличия, рейтинг
- [x] Цена / старая цена / бейдж скидки
- [x] Количество + кнопка «В корзину» + кнопка избранного
- [x] Теги (ссылки в каталог)
- [x] Таблица характеристик (specs)
- [x] Описание
- [x] Кнопка «Назад»
...
- [x] **6.4** Проверить AI-режим: Groq qwen3-32b работает, ответы корректны
- [x] **6.5** Проверить function calling: multi-turn loop, searchProducts → addToCart работает
- [x] **6.6** Проверить `addToCart` из чата — продукт найден и добавлен в корзину
...
- [x] Экспорт CSV заказов и товаров
- [x] Wishlist (избранное)
- [ ] Аутентификация B2B: реквизиты, договор, НДС
- [x] Расширение AI tools: `compareProducts`, `checkStock`, `getRecommendedByBudget`
- [ ] i18n (next-intl)
